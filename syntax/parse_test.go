...
func TestExprParseTrees(t *testing.T) {
	for _, test := range []struct {
		...
		{`[e for x in y if cond1 if cond2]`,
			`(Comprehension Body=e Clauses=((ForClause Vars=x X=y) (IfClause Cond=cond1) (IfClause Cond=cond2)))`}, // github.com/aabbtree77/starlark issue 53
	} {
...
func TestParseErrors(t *testing.T) {
	filename := starlarktest.DataFile(".", "testdata/errors.star")
