...
// Caller returns the frame of the innermost enclosing Starlark function.
...
// A Frame holds the execution state of a single Starlark function call
// or module toplevel.
type Frame struct {
...
// wrapError wraps the error in a starlark.EvalError only if needed.
...
func evalSliceExpr(fr *Frame, e *syntax.SliceExpr) (Value, error) {
	// Unlike Python, Starlark does not allow a slice on the LHS of
